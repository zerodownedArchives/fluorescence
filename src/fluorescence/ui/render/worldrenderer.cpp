...
WorldRenderer::WorldRenderer(components::WorldView* worldView) :
...
void WorldRenderer::checkTextureSize() {
...
void WorldRenderer::initFrameBuffer(unsigned int index) {
...
void WorldRenderer::moveCenter(float moveX, float moveY) {
    movePixelX_ = moveX;
    movePixelY_ = moveY;
}

CL_Texture WorldRenderer::getTexture(CL_GraphicContext& gc) {
    checkTextureSize();
...
void WorldRenderer::renderPreviousTexture(CL_GraphicContext& gc, float pixelX, float pixelY) {
...
void WorldRenderer::prepareStencil(CL_GraphicContext& gc) {
    gc.clear_stencil(0);
...
void WorldRenderer::render(CL_GraphicContext& gc) {
...
void WorldRenderer::renderParticleEffects(CL_GraphicContext& gc) {
...
void WorldRenderer::renderObject(CL_GraphicContext& gc, world::IngameObject* obj, ui::Texture* tex) {
...
void WorldRenderer::batchFlush(CL_GraphicContext& gc) {
...
void WorldRenderer::forceRepaint() {
    forceRepaint_ = true;
    texturesInitialized_[0] = false;
    texturesInitialized_[1] = false;
}

void WorldRenderer::initBufferControls() {
